...
function Question({title,desc}) {
  ...
  return (
    <div className="border-[1px]  border-green4 relative  w-full">
        ...
        <button onClick={collapse} className="collapsible w-full py-4 pl-5 pr-10 font-semibold text-xl md:text-3xl select-none">{title}</button>
        <div className={`content font-normal text-base md:text-xl px-5  ${maxHeight}`} >
         <p>{desc}</p>
        </div>
    </div>
  )
